# relationship_detail.tsv — revised semantics and filling matrix (DB2-focused, schema-aware, production-tightened v5.4)
...
## Revision scope for v5.4

This revision keeps the prior tightening and additionally closes several DB2 SQL PL gaps that still allowed two careful implementations to emit different but plausible TSV rows. In particular, this revision strengthens:
...
- explicit usage-side completion for ordinary scalar-function arguments such as `ROUND(expr, 2)`
- explicit rule that `SELECT`-list literals inside `EXISTS` / subqueries still emit normal usage-side rows such as `SELECT_EXPR`
- stronger family-level alias-preservation enforcement for predicate and select-expression families such as `WHERE`, `JOIN_ON`, `SELECT_FIELD`, and `SELECT_EXPR` when plain field tokens would be ambiguous


## v5.4 closure patch

This revision keeps the v5.3 structure but resolves the three remaining contract gaps highlighted by the deep review of `RPT.PR_TEST_DEMO.sql`, the reference TSV, and the parser TSV:
- `confidence` is now defined as **extraction-path metadata**, not as a semantic-truth score
- `FUNCTION_EXPR_MAP` is restricted to the **single outermost named function expression** whose result directly lands in the written target slot
- value-producing `CASE` expressions now explicitly require their `WHEN` condition tokens to use the surrounding expression context's usage-side family instead of `CONTROL_FLOW_CONDITION`

This v5.4 revision is intentionally conservative:
- it does **not** introduce any new relationship family
- it keeps deterministic `SELECT ... INTO` / `VALUES ... INTO` target-flow rules
- it preserves strongest-family exclusivity for direct slot mappings from special registers and sequences
- it makes semantic-gold validation and extraction-path validation separable instead of conflating them through the `confidence` column
...
Rules:
- For nested functions such as `COALESCE(F1(X), F2(Y), 0)`, operand tokens `X`, `Y`, and `CONSTANT:0` are direct usage-side participants in the outer expression. Only the **outermost expression root** may emit `FUNCTION_EXPR_MAP` when that root is itself a named function whose result directly lands in the written target slot.
- When a function call directly contributes value to a target slot, the contract must follow one uniform policy. Under the default profile in this document, emit `FUNCTION_EXPR_MAP` exactly once for the single outermost named function expression that directly lands in the written target slot, including DB2 built-in scalar functions and user-defined functions. Do not leave this to implementation preference.
- Do not emit cascading target-slot `FUNCTION_EXPR_MAP` rows for nested helpers or wrapper stacks such as `CHAR(DECIMAL(ABS(v_n),10,2))`. If the written expression root is not a function node, do not emit `FUNCTION_EXPR_MAP` merely because one or more functions appear inside the expression.
...
- When canonical `<alias>.<field>` form is required for truthful disambiguation under the active statement scope, preserve that qualified form directly in `source_field` rather than collapsing to the plain field token.
...
### `confidence`
`confidence` records the **extraction path** used to produce the row. It is **not** a semantic-truth score.

Allowed values:
- `PARSER` = produced directly by structured parsing / AST-backed / statement-aware extraction
- `REGEX` = produced by line-level or pattern-level fallback extraction
- `DYNAMIC_LOW_CONFIDENCE` = produced from unresolved or only partially recoverable dynamic SQL

Validator note:
- a semantic-gold validator may choose to ignore `confidence` entirely, or validate it under a separate extraction-path profile
- two rows can be semantically equivalent yet still differ validly or defectively in `confidence` depending on the implementation path actually used
- do not upgrade a row from `REGEX` to `PARSER` merely because the row's semantics are correct
...
13. **`FUNCTION_EXPR_MAP` vs operand-only mapping rows**
    - When a named function call contributes a direct value to a target slot, emit `FUNCTION_EXPR_MAP` exactly once for the single outermost named function expression whose result directly lands in that target slot.
    - Under the default profile, this applies to both DB2 built-in scalar functions and user-defined functions.
    - Do not emit target-slot `FUNCTION_EXPR_MAP` rows for nested helpers, wrapper stacks, or functions that appear only inside a larger non-function root such as `CASE`, concatenation, arithmetic, or boolean predicates.
...
15. **Procedural `CASE` statement vs value-producing `CASE` expression**
    ...
    - For a value-producing `CASE`, the direct participating tokens inside each `WHEN` condition must use the surrounding expression context's usage-side family, such as `SELECT_EXPR`, `UPDATE_SET`, or `RETURN_VALUE`; they must **not** use `CONTROL_FLOW_CONDITION`.
    - Functions that appear only inside a value-producing `CASE` condition remain usage-side participants only and must not emit target-slot `FUNCTION_EXPR_MAP` unless the entire written expression root is itself a function.
...
- For a value-producing `CASE` inside variable assignment, `WHEN` condition tokens are usage-side only and therefore use `SELECT_EXPR`; only branch result values may produce `VARIABLE_SET_MAP` rows under the default profile.
...
### 6.x.7 Ordinary scalar-function argument completeness

The direct-expression completion rules in this contract apply to ordinary scalar function calls as well, not only to `CASE`, `COALESCE`, null checks, and concatenation.

Rules:
- Every direct participating argument token of an ordinary scalar function call must emit the normal usage-side row required by the surrounding expression context.
- This includes numeric literal arguments such as precision, scale, rounding digits, substring positions, and similar direct literal operands.
- Do not suppress a literal argument merely because it is not the first function argument.
- `FUNCTION_EXPR_MAP` or other mapping-side rows do not replace these required usage-side rows.

Examples:
- `ROUND(AMT * RATE, 2)` must emit a usage-side row for `CONSTANT:2` in addition to the usage rows for `AMT` and `RATE`.
- `SUBSTR(V_CODE, 1, 3)` must emit usage-side rows for `CONSTANT:1` and `CONSTANT:3` when those literals directly participate in the expression.

Required usage relationship by context:
- `SELECT_EXPR` when the function call appears in a `SELECT` item or scalar value-producing expression governed by the contract's generic value-expression usage policy
- `UPDATE_SET` when the function call appears in an `UPDATE SET` expression
- `RETURN_VALUE` when the function-call arguments are direct participating tokens of a scalar `RETURN` expression

### 6.x.8 Subquery select-list literal participation

A `SELECT`-list literal inside a subquery remains a direct SQL token and must follow the normal usage-side rules for `SELECT` items.

Rules:
- `SELECT 1`, `SELECT 'X'`, `SELECT CURRENT DATE`, and similar select-list literals inside scalar subqueries, `EXISTS`, `IN`, or correlated subqueries must emit the normal usage-side row for that token.
- The presence of `EXISTS` does not suppress the select-list token merely because the database engine may not need the runtime value.
- Use `SELECT_EXPR` for such select-list literals unless a stronger explicit relationship family applies.

Example:
- In `WHERE EXISTS (SELECT 1 FROM T WHERE T.ID = X.ID)`, emit a `SELECT_EXPR` row for `CONSTANT:1` anchored to the physical line containing `SELECT 1`.

### 6.x.9 Validator clarification for scalar-function literal operands and subquery select-list literals

A validator should treat the following as defects:
- missing usage-side rows for direct literal operands such as the `2` in `ROUND(expr, 2)`
- missing usage-side rows for direct subquery select-list literals such as `SELECT 1`
- alias-qualified predicate or select-expression tokens collapsed to plain field tokens when canonical `<alias>.<field>` form was required for truthful disambiguation
...
- This same preservation requirement applies to predicate families such as `WHERE` and `JOIN_ON`, and to select-expression families such as `SELECT_FIELD` and `SELECT_EXPR`, whenever the plain field token would be ambiguous in the active statement scope.
...
This closure patch turns the last remaining validation ambiguities into explicit normative text.

It specifically:
- makes ordinary scalar-function argument completeness explicit, including literal operands such as the `2` in `ROUND(expr, 2)`
- makes `SELECT`-list literals inside `EXISTS` and other subqueries explicit usage-side participants
- extends family-level alias-preservation enforcement to predicate and select-expression families when plain field tokens would be ambiguous
- adds validator wording so these cases are rejected consistently

No new relationship family is introduced in v5.3.
